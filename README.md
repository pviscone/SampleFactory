...
- `--njobs` : Number of jobs to submit. Hence, the sample production will in the end return `nevents x njobs` events in total (The job will produce max `nevents`, stopping when and if `nout` is reached).
